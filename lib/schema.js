...
const debug = require('debug')('appsync:vtl')
const vtlMacros = {
  console: (...args) => {
...
const buildVTLContext = (
  ...
  const vtlContext = {
    ...
    prev: result ? { result: JSON.stringify(result) } : undefined,
    source: root || {},
...
const returnJSON = input => {
  try {
    ...
    consola.error('Failed to parse VTL template as JSON (see below)\n', input)
...
const runRequestVTL = (fullPath, graphqlInfo, result = null, stash = null) => {
  ...
  debug(`Rendering ${path.relative(process.cwd(), fullPath)}`)
  return [
    handleVTLRender(content.toString(), vtlContext, vtlMacros, graphqlInfo),
...
const runResponseVTL = (fullPath, graphqlInfo, result, stash) => {
  ...
  const vtlContext = buildVTLContext(graphqlInfo, result, stash)
  const content = fs.readFileSync(fullPath, 'utf8')
  return handleVTLRender(content.toString(), vtlContext, vtlMacros, graphqlInfo)
}
...
const generatePipelineResolver = ({ before, after, functions }) => {
    ...
    (sum, { dataSource, type, field, request, response, kind, functions }) => {
      ...
      if (kind && kind.toUpperCase() === 'PIPELINE') {
        const fns = functions.map(f => {
          ...
          if (!functionDef)
            throw new Error(`Pipeline function ${name} not defined`)
