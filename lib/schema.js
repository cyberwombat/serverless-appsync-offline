...
const generatePipelineResolver = ({ before, after, functions }) => {
  return async (root, vars, context, info) => {
    ...
    // stash.put('clientId', '75a23f82-35f3-4c3f-aea2-bb782d7ebe5a')

    const vtlContext = buildVTLContext(resolverArgs, null, stash)

    let pipeResult
    let pipeStash = stash
    let pipeResponse
    for (let fn of functions) {
      pipeResult = await fn(root, vars, context, info, null, pipeStash)
