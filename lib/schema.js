...
const handleSubstitutions = (str, subs = {}) => {
  return str.replace(/\$\{(\w+)\}/g, (match, p1, p2) => subs[p1]
}
...
const handleVTLRender = (
  ...
  try {
    templateOutput = vtl(handleSubstitutions(str.toString(), config.substitutions), context, vtlMacros)
  } catch (err) {
...
const runRequestVTL = (fullPath, graphqlInfo, result = null, stash = null, config) => {
  ...
  return [
    handleVTLRender(handleSubstitutions(content.toString(), config.substitutions), vtlContext, vtlMacros, graphqlInfo, config),
...
const runResponseVTL = (fullPath, graphqlInfo, result, stash, config) => {
  ...
  const vtlContext = buildVTLContext(graphqlInfo, result, stash)
  const content = fs.readFileSync(fullPath, 'utf8')
  return handleVTLRender(content.toString(), vtlContext, vtlMacros, graphqlInfo, config)
}
...
const generateTypeResolver = (
  ...
  try {
    ...
    const [request, stash] = runRequestVTL(requestPath, resolverArgs, null, null, config)
...
const generatePipelineResolver = ({ before, after, functions, config }) => {
  return async (root, vars, context, info) => {
    ...
    assert(context && context.jwt, 'must have context.jwt')
    const resolverArgs = { root, vars, context, info }
    const [request, stash] = runRequestVTL(before, resolverArgs, null, null, config)

    let pipeResult
    let pipeStash = stash
    let pipePayload = null
    let pipeResponse
    let pipeContext = vars
    ...
    (sum, { dataSource, type, field, request, response, kind, functions }) => {
      ...
      if (kind && kind.toUpperCase() === 'PIPELINE') {
        const fns = functions.map(f => {
          ...
          if (!functionDef)
            throw new Error(`Pipeline function ${f} not defined`)
        ...
        resolver =
          type === 'subscription'
            ? generateSubscriptionTypeResolver(field, source, config, configs, pathing)
